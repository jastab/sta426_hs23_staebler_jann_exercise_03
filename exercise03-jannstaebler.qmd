...
```{r }
#| ouput: false
library(dplyr) # for data wranling
```
...
```{r}
reps <- data.frame(anno %>% group_by_all() %>% count) # grouping by treatment / technology combination and count the replicates (replicate=same combination of treatment and technology)
```

Following a list of the repilcates (same treatment and technology) and number of replicates n:.
...
testcount
normalized_testdf <- sweep(testdf,2,testcount,FUN="/") # now we divide each value by the corresponding sum of the column it is in, so we should get a sum of 1 for each column if we then sum up the columns again
normalized_testdf
...
```{r}
log_normalized_df_x <- log1p(normalized_df_x) #quite straigth forward. log1p(x) computes log(1+x), so log1p(0)=0 
```
